...
const makeFetchRequest = () => {
  ...
  function replaceTextElements() {
    ...
    paragraphs.forEach((paragraph, index) => {
      ...
      if (paragraphContent) {
        // stay under defined character count
        const paragraphLength = paragraphContent.length;
    ...
    const extractedParagraphsString = extractedParagraphs.map((paragraph) => {
      return `${paragraph.id}: ${paragraph.content}`;
    }).join("\n");

    console.log("Finished extracting paragraphs");
    console.log(extractedParagraphsString);

    // send to openai
    const apiKey = "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"
    const apiUrl = "https://api.openai.com/v1/chat/completions"

    let prompt = "Hello";
    prompt = "You are a bot that rewrites content. You maintain the format, rewriting the content in place, aiming for a similar number of characters for each section. Maintain the same format as the input, with ids. Keep the same ids. If there is a blank, leave it there. Do NOT add headers or anything else. Please rewrite this content to match a 10th grade reading level:\n"
    ...
    if (response) {
      ...
      paragraphs.forEach((paragraph, index) => {
        if(index < responseParagraphs.length) {
          
          // split at first colon to get the ID
          const splitText = responseParagraphs[index].split(': ');
          console.log(splitText);
          // rejoin the rest since there may be more than one colon
          let replacementText = splitText.slice(1).join(': '); 
          console.log("ID: " + index + ", " + replacementText)
    ...
    //         // add links from original paragraph to bottom of new paragraph
    //         const links = paragraph.querySelectorAll('a');
    //         links.forEach((link) => {
